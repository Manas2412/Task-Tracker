...
export function WeekView({ grid, events }: WeekViewProps) {
  ...
  return (
    <div className="rounded-xl border border-line bg-panel overflow-hidden">
      {/* Weekday header strip — the single date row for every breakpoint. */}
            ...
            <div key={d} className="px-1 py-2 text-center">
              ...
              >
                {cell.isToday ? (
                  ...
                  <span className="text-[17px] md:text-[18px] font-medium leading-none text-ink tabular-nums">
                    {cell.date.getDate()}
                  </span>
                )}
                {count > 0 ? (
                  <span className="text-[10px] text-ink-3 font-medium leading-none tabular-nums">
                    {count}
                  </span>
                ) : null}
              </DayCellButton>
      ...
      {/* Week day columns — chips only; the date lives in the header strip above. */}
        ...
        {grid.map((cell, i) => {
          ...
          return (
            <div
              key={i}
              className={cn(
                'border-r border-line-2 min-h-[160px] md:min-h-[200px] p-1 md:p-1.5 flex flex-col gap-0.5',
                i === 6 && 'border-r-0',
              )}
            >
              {dayEvents.map((e) => (
