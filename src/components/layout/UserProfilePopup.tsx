...
import type { UserProfileCard } from '@/lib/user-profile';
  ...
  useEffect(() => {
    ...
    fetch(`/api/users/${userId}/profile`, { signal: ctl.signal, cache: 'no-store' })
      .then(async (r) => {
        ...
        return (await r.json()) as UserProfileCard;
      })
      .then((data) => {
        setProfile(data);
        setLoading(false);
    ...
    <>
      ...
      <div
        ...
        className={cn(
          ...
          'md:inset-x-auto md:bottom-auto md:left-1/2 md:top-1/2 md:w-[440px]',
  ...
  const inner = (
    <>
      ...
      <span className="min-w-0">
        ...
        <span className={cn('block truncate text-[13px]', value ? 'text-ink' : 'text-ink-3 italic')}>
          {value ?? 'Not provided'}
        </span>
      </span>
    </>
