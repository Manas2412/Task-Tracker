...
### 5.19 PMU team-leader admin scope

A PMU **Team Leader** (`users.pmu_role = 'pmu_team_leader'`, active) is the admin of their **PMU team only** — the team being every active PMU member (`is_pmu = true`) sharing the leader's division and PMU attachment (`pmu_id`, matched as-is so the common "flagged member in a normal division, `pmu_id = null`" case groups by division). The team set is resolved by `getPmuTeamMemberIds(userId)` (`src/lib/pmu-team.ts`), which returns `[]` for anyone who is not an active team leader, so the scope is inert for every other user.

- **Read.** The leader additionally sees their team's **division-visibility** tasks. `buildVisibilityClausesFrom` (`src/lib/visibility-rules.ts`) emits an owner-scoped clause `{ visibility: 'division', ownerId: { in: <team ids> } }` from `opts.pmuTeamLeaderMemberIds`, populated by `buildVisibilityClauses`. A teammate's **personal** task stays private (the clause is `division`-scoped).
- **Manage.** Over a **division** task **owned by a team member**, the leader may edit status / priority / context, manage subtasks, and add / remove collaborators (`canManageTask`'s `pmuTeamMemberIds` branch, gated on `visibility === 'division'`), attach / rename / delete documents (`canEditTaskAttachments`), and **reassign** the owner — freely to another team member, otherwise via the normal transfer matrix (`reassignTaskAction`).
- **Scope limits — the leader never gains:** any power over the division's **non-PMU ministry tasks** (owner not on the team) or another PMU / division; **delete** (`deleteTaskAction` stays on `canActAsHeadOf`); **redefine** (name / due date / recurrence — `canEditTaskDetails`); or **visibility / division** change. This is management scoped to the team's board tasks, never head power.
- The grant flows through the shared `canManageTask` / `canEditTask` gates, so every task-management server action honours it consistently, and the task-detail and task-list UIs mirror it exactly.
