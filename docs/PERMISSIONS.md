...
- **Cross-division allocation link (Khelo India → NSDF)** — on top of the base matrix, a **Khelo India division head** (direct head or active delegate) may **allocate** tasks to **NSDF members**, where "allocate" covers all three ways of putting work on a user:
  - **Create** a `division`-visibility task for NSDF (Quick Create and Create-from-Timeline-File offer NSDF as a target with its members as owners; `createTaskAction` / bulk import accept it) — via `canCreateDivisionTask`.
  - **Assign / reassign** an owner directly to an NSDF member from the Owner row of a task the KI head may already reassign (their own division's tasks) — via `canAssignTaskTo`; the reassignment picker surfaces those members (`canTransferTaskToOrLinked`) and the assignment is free (no approval).
  - **Transfer** to an NSDF member (unchanged) — via `canTransferTaskToOrLinked`.

  The link grants **allocation reach only, not head powers** over NSDF: the KI head cannot delete NSDF's tasks, does not see NSDF's board, cannot manage NSDF's existing tasks (unless they created them), and cannot delegate NSDF. The reassignment **approval** matrix (§5.6) is unchanged — cross-division owner *proposals* that need approval still follow the base rule (but the KI→NSDF assignment above is free, so no proposal is created). Links are configured by division **abbreviation** (`KI`, `NSDF`) in `CROSS_DIVISION_ALLOCATION_LINKS` (`src/lib/rbac/index.ts`) and resolved to ids per request; add more pairs there. Abbreviation is used rather than the display name because it has no admin edit path (a rename cannot silently break the link), and resolution is fail-closed (an unknown abbreviation only removes the extra reach, never grants more). It is one-way — an NSDF head gains no reciprocal reach into Khelo India. Visibility: a task the KI head creates for an NSDF owner then lives on NSDF's board; the KI head keeps management rights as its creator AND can still see/track it (the visibility scoper grants sight of division tasks the caller CREATED in a linked division), but this never exposes the rest of NSDF's board — only the tasks the KI head themselves put there.
...
- **Pull (claim ownership).** Any active user **in the task's own division** may pull an unassigned, non-personal, top-level task and become its owner — `pullTaskAction`, surfaced as the **Pull task** button on the task detail page (shown when `ownerId === createdById`, the viewer is not already the owner, and the task is a non-personal top-level task in the viewer's division). Pulling logs a `task_pulled` activity event and notifies the creator.
...
### 5.17 Cross-division participant links + document at subtask creation

Two enhancements sit on top of the base rules:

- **Cross-division participants (Khelo India / Khelo India Mission ↔ NSDF).** On top of the participant rule (§5.3), members of a **cross-linked division** count as participants of the task and so may be added as **collaborators**, assigned **subtasks**, and **@mentioned in the discussion** — everything the participant set gates. The links are **symmetric** and open to **all members**, not just heads: a Khelo India or Khelo India Mission task may pull in an NSDF member, and an NSDF task may pull in a Khelo India / Khelo India Mission member. The reach is **participant-only** — it grants **no** head powers, no visibility of the other division's board, and does not touch the task **owner** rule (owner stays same-division at creation — §5.14). A cross-division member sees the specific task they were added to via the owner / collaborator / mention clauses (§5.1), never the wider board. Configured by division **abbreviation** in `CROSS_DIVISION_PARTICIPANT_LINKS` (`src/lib/rbac/index.ts`); resolution is fail-closed (an unknown abbreviation drops the pair). Folded into `buildTaskParticipantWhere` / `isTaskParticipant` (`src/lib/task-participants.ts`), so every picker and its server guard — `addCollaboratorAction`, `addSubtaskAction` / `updateSubtaskAction`, `resolveMentions` — inherit it uniformly. Distinct from the head-only **allocation** link of §5.6, which governs top-level task creation / assignment / transfer.
