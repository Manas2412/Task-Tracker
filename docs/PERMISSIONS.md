...
### 5.17 Cross-division subtask links + document at creation

Two subtask-creation enhancements sit on top of the base rules:

- **Cross-division subtask assignees (Khelo India / Khelo India Mission ↔ NSDF).** On top of the participant rule (§5.3), whoever may add a subtask (owner / creator / collaborator / head — §5.15) may assign that subtask to a member of a **cross-linked division**. The links are **symmetric** and open to **all members**, not just heads: a Khelo India or Khelo India Mission task may hand a subtask to an NSDF member, and an NSDF task may hand one to a Khelo India / Khelo India Mission member. This widens **only** the subtask assignee set — collaborators, @mentions, and the task owner keep the stricter base participant rule, and it grants **no** head powers or visibility of the other division's board (the assignee sees the subtask because they own it, per §5.1). Configured by division **abbreviation** in `CROSS_DIVISION_SUBTASK_LINKS` (`src/lib/rbac/index.ts`); resolution is fail-closed (an unknown abbreviation drops the pair). Enforced by `isSubtaskAssigneeAllowed` in `addSubtaskAction` / `updateSubtaskAction`, and mirrored in the picker via `buildSubtaskAssigneeWhere` (both `src/lib/task-participants.ts`). Distinct from the head-only **allocation** link of §5.6, which governs top-level task creation / assignment / transfer.
