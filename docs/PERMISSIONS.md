...
- **Cross-division transfer link (Khelo India → NSDF)** — on top of the base transfer matrix, a **Khelo India division head** (direct head or active delegate) may **transfer** tasks to **NSDF members**. This applies to the **Transfer task** flow only (the picker `fetchTransferTargets` and `transferTaskAction`, via `canTransferTaskToOrLinked`); the reassignment approval matrix (§5.6, Owner row) and every other role/division are unchanged. Links are configured by division **abbreviation** (`KI`, `NSDF`) in `CROSS_DIVISION_TRANSFER_LINKS` (`src/lib/rbac/index.ts`) and resolved to ids per request; add more pairs there. Abbreviation is used rather than the display name because it has no admin edit path (a rename cannot silently break the link), and resolution is fail-closed (an unknown abbreviation only removes the extra reach, never grants more). It is one-way — an NSDF head gains no reciprocal reach into Khelo India.
